...
class Podmornice
{
    ...
    Dictionary<int, int> brod;
    Dictionary<int, int> nepogodjeni;
    ...
    public Podmornice(string[] a)
    {
        ...
        brod = new Dictionary<int, int>();
        nepogodjeni = new Dictionary<int, int>();
        brCiljeva = 0;
        for (int i = 0; i < brRedova; i++)
        {
            for (int j = 0; j < brKolona; j++)
            {
                ...
                if (zadato[i, j])
                {
                    ...
                    if (j > 0 && zadato[i, j - 1])
                        brod[k] = brod[k - 1];
                    else if (i > 0 && zadato[i - 1, j])
                        brod[k] = brod[k - brKolona];
                    else
                    {
                        brod[k] = k;
                        nepogodjeni[brod[k]] = 0;
                    }
                    nepogodjeni[brod[k]]++;
                }
            }
